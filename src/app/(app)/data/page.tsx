  ...
  return (
    <Box my={2} gap={2} display="flex" flexDirection="column">
      <FormControl fullWidth>
        ...
        >
          <MenuItem
            key={DataPersistenceMode.NONE}
            value={DataPersistenceMode.NONE}
          >
            Keine Datenspeicherung
          </MenuItem>
          <MenuItem
            key={DataPersistenceMode.LOCAL}
            value={DataPersistenceMode.LOCAL}
          >
            Lokale Speicherung
          </MenuItem>
          <MenuItem
            disabled={data == null}
            key={DataPersistenceMode.SERVER}
            value={DataPersistenceMode.SERVER}
          >
            Server-Speicherung
          </MenuItem>
        </Select>
      ...
      {config?.dataPersistenceMode === DataPersistenceMode.SERVER && (
                ...
                <Grid key={ds.id} size={{ xs: 12, md: 6 }}>
                  <TransactionDataCard
                    icon={
                      <Avatar
                        sx={{
                          bgcolor: ds.valid ? "secondary.main" : "error.main",
                        }}
                      >
                        {ds.valid && <ShowChart />}
                        {!ds.valid && <Lock />}
